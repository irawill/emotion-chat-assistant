...
基于 **Vite + React + TypeScript** 开发的智能情感对话助手应用，能够识别用户情绪并提供相应的情感支持和对话反馈。现已支持 OpenAI API 集成、流式输出和 Markdown 渲染！
...
- **🆕 流式输出** - 实时显示 AI 响应，提供更流畅的用户体验
- **🆕 Markdown 渲染** - 支持丰富的文本格式，包括代码高亮、表格、列表等
...
- **API 模式**：通过配置的 API 端点获得智能对话响应，支持流式输出
...
│   │   └── MarkdownMessage.tsx  # Markdown 消息渲染组件
│   ├── context/          # React Context
│   │   └── ChatContext.tsx      # 聊天状态管理
│   ├── services/         # 服务层
│   │   └── api.ts              # API 请求服务（支持流式）
...
在 API 模式下，助手的回复支持完整的 Markdown 格式：
